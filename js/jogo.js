...
//armazenamos a imagem do Smile na variável imagem (getElementById)
let imagemn = document.getElementById("imagemn");
//se a imagem nao for vazia (se ela existir)
if (imagemn != "") {
  //removemos a imagem do Smile
  imagemn.remove();
...
function errou(obj) {
  ...
  obj.className = "errou";
  //Criar uma constante img que armazena um novo objeto imagem com largura de 100px
  const imgf = new Image(100);
  imgf.id = "imagemn";
  //altera o atributo src (source) da imagem criada
  imgf.src = "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQTlA5EO1JUZy9S6Ja3lUX_F2Cwz2ruLdAecg&s";
  //adiciona a imagem criada na div (obj) escolhida pelo jogador (appendChild)
  obj.appendChild(imgf);
}
...
function verifica(obj) {
  ...
  if (jogar) {
    ...
    if (obj.id == sorteado) {
      ...
      obj.className = "errou";
      // Eai professor, eu vi ali que a função verifica não chamava o errou(obj) quando errava dai eu coloquei aqui.
      errou(obj)
